      ...
      <div class="upload-content">
        <div class="upload-content-top">
            <div class="drag-wrapper" ref="drag" id="drop_area">
                ...
                <span>请将文件拖到此处或<el-button type='text' class="drag-wrapper-btn"  @click="proxyClick">点击上传</el-button></span>
                <span>当前仅支持{{typeName}}格式文件，其他文件将会过滤</span>
                <input type="file" id="uploadInput" multiple="multiple" :accept="accept" @change="tirggerFileAll($event)" />
            </div>
            <!-- 上传的是图片的时候 -->
            <div class="successImg" v-if="successImgs.length && type == 1">
...
export default {
  ...
  methods: {
    ...
    removeImg(index){
      ...
      this.$notify({title: '移除成功',message: `已经移除该${className}了！`,  type:'success'});
    }, //删除预览图片
    ...
    readAndPreview(file) {
      ...
      reader.onload = ((e) => { //简单图片读取完成之后把它分别放入预览图片数组和上传图片数组
        if (this.previewImgsArr.length > 4) { //因为判断在添加完之前 所以这里的4实际是五张图
            this.$notify({title: `${className}超限`,message: `最多一次五份${className}哟`,  type:'warning'});
        } else {
            if (this.previewImgsArr.indexOf(e.target.result) == -1) {
                this.previewImgsArr.push(e.target.result);
                this.uploadFileArr.push(file);
            } else {
                this.$notify({title: `重复${className}`,message: `已经过滤掉重复${className}啦！`,  type:'warning'});
            }
        }
      });
    ...
    openDrawer(type) {
      ...
      setTimeout(() => {
        ...
        dp.addEventListener("drop", function (e) {
          ...
          [].forEach.call(articles, _vm.readAndPreview);
        });
      }, 100);
